...
export const getAddressSearch = async (req, res) => {
    ...
    try {
        if (text.data?.length > 2) {
            ...
            const defaultData = await instance.get(`/geocoding/v1/search?text=${value}&lang=en&sources=oa%2Cosm%2Cnlsfi`);
            const transportData = await instance.get(`/geocoding/v1/search?text=${value}&lang=en&sources=gtfsHSL%2CgtfsHSLlautta`);
            const combinedData = [
...
export const getAddressLookup = async (req, res) => {
    ...
    try {
        const addressLookupData = await instance.get(`/geocoding/v1/reverse?point.lat=${data.data.latitude}&point.lon=${data.data.longitude}&lang=en&size=1&layers=address`);
